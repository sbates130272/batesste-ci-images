...
then fails the DKMS build. The flavour therefore pins **noble +
mainline v7.2.3** — `release: noble`, `kernel_ref: v7.2.3` in `images.yml`.

That combination is not a guess: it is what rocm-ernic's self-hosted golden
image is already running and passing on, down to the `202609030700` build
stamp. It was installed there by hand and recorded nowhere, which is why the
two self-hosted guests have since drifted onto different kernels. Pinning it
in the catalogue is the point of this flavour.

The distro release does not constrain the kernel. Ubuntu's mainline builds are
published once per version, not per release, and depend only on a handful of
base packages that noble already satisfies — so noble is chosen for its
userspace, and 7.2.3 arrives independently of it.
...
`ernic_vm_release: noble` is right and does not change. What is missing there
is the mainline kernel, which nothing in rocm-ernic installs — `kernel_setup`
builds from git.kernel.org, and the only mention of the mainline PPA is prose
in `docs/ionic.rst`. Consuming this image supplies it.
...
hand-installed mainline kernel and a source-built rdma-core 62.0 overwriting
...
the mainline kernel exists, so `linux-headers-generic` would pull noble's 6.8
headers and DKMS would build against the wrong tree.
...
- noble's apt `rdma-core` is far older than `providers/ionic` (upstream v61),
  so a job needing `libionic*.so` must still build rdma-core from source over
  the top. That build overwrites dpkg-owned files under `/usr`, so hold or
...
- `IONIC_KERNEL_REF` is pinned at `v7.2.4` and the guest kernel is 7.2.3: one
  point release apart, which is the skew already proven to work. Keep the two
  moving together — a kernel below 7.2 fails on `ib_umem_get_va`.

## Acceptance

1. `./ci-images-tool.py build ubuntu-qcow2-gen@ionic` passes, including the
   probe boot running `checks/ionic.sh`.
2. `vm-info.json` reports `flavour: ionic`, `release: noble`,
